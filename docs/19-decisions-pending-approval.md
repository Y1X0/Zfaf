...
## 6. قرارات معلّقة ظهرت أثناء التنفيذ

هذه لم تكن معروفة في Phase 0 وظهرت عند بناء الكود. **لم يُنفَّذ فيها أي سلوك مخترَع.**

### 🟡 PD-01 — المصادقة: تعارض بين Auth.js و ADR-0006

📄 **[ADR-0018](adr/0018-first-party-authentication.md)** — يحتاج قرارك.

محوّل Prisma الخاص بـ Auth.js يخزّن رموز الجلسات **خاماً** ولا يدعم الإبطال، وهو ما يلغي
ADR-0006 من أساسه. نُفِّذت المصادقة على مخططنا محافظةً على ADR-0006، و**Google OAuth مؤجَّل**
حتى تُقرّ الـ ADR وتختار مكتبة OAuth.

**مكتشَف في:** M2 · **الأثر:** `D2.5` غير منفَّذ · **لا يحجب M3**

---

### 🟡 PD-02 — ماذا يحدث لدعوة منشورة عند تعطيل حساب صاحبها؟

**الوضع في الوثائق:**

| الحالة | مقرَّرة؟ | السلوك |
|--------|:--------:|--------|
| حذف الحساب | ✅ [09 §8](09-auth-and-rbac.md#8-حذف-الحساب) | الدعوات → `PAUSED` فوراً |
| **تعطيل الحساب (suspension)** | ❌ **غير مقرَّر** | — |

**ما نُفِّذ (المقرَّر فقط):** المستخدم المعطَّل لا يستطيع المصادقة ولا أي إجراء —
`tenantScopeFor` يرجع `null` و`can()` يرفض كل شيء بـ `ACCOUNT_SUSPENDED`.

**ما لم يُنفَّذ:** أي تغيير في ظهور دعواته المنشورة. **الوضع الحالي: تبقى ظاهرة.**
هذا ليس قراراً — إنه غياب قرار، ومُعلَّم صراحةً في الكود والاختبارات.

**الخيارات:**

| # | الخيار | الحجّة | الحجّة المضادة |
|---|--------|--------|-----------------|
| أ | تبقى الدعوات ظاهرة | التعطيل عقوبة على **المستخدم** لا على 250 ضيفاً؛ متسق مع مبدأ «لا نعاقب الضيوف» في [11 §7](11-payments-architecture.md) | لو كان التعطيل بسبب **محتوى مسيء**، يبقى المحتوى منشوراً |
| ب | تُوقَف كل الدعوات تلقائياً | يوقف الضرر فوراً | يعطّل عرساً قد يكون بعد يومين بسبب نزاع غير متعلق به — ضرر لا يُعوَّض ([خطر O1](18-risks.md)) |
| ج ⭐ | التعطيل لا يمسّ الدعوات؛ المحتوى المسيء يُعالَج بـ **kill switch على الدعوة** المستقل | يفصل عقوبة الحساب عن إزالة المحتوى — أداتان لمشكلتين مختلفتين | يتطلب من الأدمن إجراءين |

**توصيتي: (ج).** الأداتان موجودتان بالفعل ومنفصلتان في المخطط (`users.status`
و`invitations.status='SUSPENDED'`). دمجهما يخلط عقوبةً بإزالة محتوى، ويجعل خطأً في الأولى
يسبب ضرراً غير قابل للتعويض في الثانية.

**مكتشَف في:** M2 · **الأثر:** لا سلوك منفَّذ · **لا يحجب M3**
